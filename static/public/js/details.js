...
function getPoints(param){
    $.get("/getPoints?areaName="+param,"",function(data){
        console.log(data);
    // $.get("/details/getPoints?areaName="+param,"",function(data){
        var all = "<option selected=selected value='全部'>全部</option>";
        $("#point").append(all);
...
function initDetailsData(){
     $('#list_table').bootstrapTable({
        ...
        onLoadSuccess:function(value){
            $('#list_table').bootstrapTable('load',value);
        }
    })
}

function queryParams(params) {   //searchText: undefined, sortName: "id", sortOrder: "desc", pageSize: 15, pageNumber: 1  
        console.log(111111111111111111111,"pointName:"+$("#point").val(),"areaName:"+$("#areaName").val());
    var args = {
        // pointName: pointName,
        // areaName : areaName,
        pointName: $("#point").val(),
...
//    var endTime = $("#endTime").val();
    ...
    var url="exportDetails?areaName="+areaName
...
//添加查询时间限制
var countdown=10;
function settime(val) {  //<button>  </button>
	if(countdown != 0){
        val.setAttribute("disabled", true);   //禁用按钮
